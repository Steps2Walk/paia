...
ui = fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(width = 4,
                 ...
                 p("Annotate the scale, positional and calcaneal tuberosity markers"),
                 p("Click calculate PAIA to calculate the most optimum insertion angle"),
                 fileInput("file", "Upload an X-ray Scan", accept = c("image/jpeg")),
                 selectInput("color", "Select Annotation Type", 
                             choices = c("Positional" = "#7570B3", 
                                         "Scale" = "#1B9E77", 
                                         "Calcaneal Tuberosity" = "#D95F02"), 
                             selected = "Scale"),
                 helpText("Click and drag to create a blue rectangle for easier positional annotation"),
                 actionButton("clear", "Clear All Annotations"),
                 br(),br(),
                 verbatimTextOutput("click_info"),
                 actionButton("run", "Calculate PAIA"),
                 hr(),
                 p("For more instructions, please refer to the",  
                   a("GitHub repo", href = "https://github.com/Broccolito/paia", 
                     target = "_blank"), ".")
    ),
    mainPanel(
